...
_LEGAL_PREFIXES = (
    ...
    # Полные формы (как Brizo записывает из ЕГРЮЛ)
    "ОБЩЕСТВО", "АКЦИОНЕРНОЕ", "ПУБЛИЧНОЕ", "НЕПУБЛИЧНОЕ",
    "ЗАКРЫТОЕ", "ОТКРЫТОЕ", "ГОСУДАРСТВЕННОЕ", "МУНИЦИПАЛЬНОЕ", "ФЕДЕРАЛЬНОЕ",
    "НЕКОММЕРЧЕСКОЕ", "АВТОНОМНАЯ", "ПРОИЗВОДСТВЕННЫЙ",
)
